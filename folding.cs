...
public static class Folding {
    ...
    public class MyList<A> {
        private readonly int headIndex = 0;
        private readonly A[] items;
        public MyList(params A[] items) { this.items = items; }
        private MyList(int headIndex, params A[] items) { 
            this.headIndex = headIndex;
            this.items = items; 
        }
        public bool IsEmpty() { return headIndex >= items.Length; }
        public A Head() { return items[headIndex]; }
        public MyList<A> Tail() { return new MyList<A>(headIndex+1, items); }
    }

    public static A FoldLeft<A,B>(Func<A, B, A> f, A accum, MyList<B> list) {
    ...
    public static A FoldRight<A,B>(Func<B,A,A> f, A seed, MyList<B> list) {
        if (list.IsEmpty()) return seed;
